const model = (function() {
  ...
  return {
    getRandomCards: function() {
      for (let i = 0; i < controller.numberOfCards / 2; i++) {
...
const view = (function() {
  ...
  const HTMLCardTemplate =
    '<div class="card"><div id="%0%" class="card__side card__side--front">&nbsp;</div><div class="card__side card__side--back">%hex%</div></div>';
  ...
  return {
    ...
    showCards: function(cardList) {
      ...
      for (let i = 0; i < controller.numberOfCards; i++) {
        let newCard = HTMLCardTemplate.replace('%0%', i).replace(
          '%hex%',
          cardList[i],
        );
      ...
      for (let y = 0; y < controller.numberOfCards; y++) {
    ...
    updateScore: function(counter) {
      document.querySelector('.sidebar__score-number').textContent =
        12 - counter * 2;
    },
